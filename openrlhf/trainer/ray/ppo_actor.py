...
from openrlhf.datasets import PromptDataset, SFTDataset, PromptLabelDataset
from openrlhf.models import Actor
from openrlhf.trainer import PPOTrainer
from openrlhf.trainer.ppo_utils import Experience, RemoteExperienceMaker, RemoteExperienceMakerWithLabel
...
class ActorPPOTrainerWithLabel(ActorPPOTrainer):
    def __init__(
            self,
            *args,
            vllm_engines: List = None,
        ...
        @Fangkai
        The only difference is that this class uses RemoteExperienceMakerWithLabel instead of RemoteExperienceMaker
        """
        super().__init__(*args, **kwargs)
        self.remote_rm_url = remote_rm_url
        self.vllm_engines = vllm_engines
        ...
        self.experience_maker = RemoteExperienceMakerWithLabel(
            self.actor,
            self.critic,
            self.reward_model,
            self.initial_model,
            ...
            vllm_engines=self.vllm_engines,
            packing_samples=self.strategy.args.packing_samples,
            use_gen_rm=getattr(self.strategy.args, "use_gen_rm", False),
        )
    ...
    def _save_checkpoint(self, args, tag, client_states):
        ...
        self.strategy.save_model(
            self.actor,
            self.tokenizer,
            os.path.join(args.save_path, 'checkpoint-{}'.format(tag.replace("global_step", ""))),
        )
...
@ray.remote(num_gpus=1)
class ActorModelRayActorLabelReward(BasePPORole):
    def init_model_from_pretrained(self, strategy: DeepspeedStrategy, pretrain):
        args = strategy.args
    ...
    def prepare_datasets(self):
        ...
        # prepare datasets
        self.prompts_dataset = PromptLabelDataset(
            args.prompt_data,
            self.tokenizer,
            strategy,
    ...
    def fit(
        ...
        args = self.strategy.args
        vllm_gen_kwargs = {}
        if "vllm_stop_token_ids" in args and args.vllm_stop_token_ids:
            vllm_gen_kwargs = {
                "stop_token_ids": list(map(int, args.vllm_stop_token_ids.split(",")))
            }

        # configure Trainer
        trainer = ActorPPOTrainerWithLabel(
            strategy,
            self.actor,
            critic_model,
            reward_model,
